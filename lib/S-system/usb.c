...
INT8U MaxGroupNumber_is_group_num = False;
...
INT8U is_language_password = True;
#endif

#if defined(FULGENCY_FUN)
INT8U CurrentPrintMode = 0;
#endif
#ifdef DYSS_UV_CLEAN
...
USBDDriver usbdDriver FAST_DATA_SECTION;
...
void USBDMA_EP2_DoneInterrupt(void) FAST_FUNC_SECTION
{	
	OS_CPU_SR cpu_sr;
	OS_ENTER_CRITICAL();	
	TickAfterLastComm = 0;
...
void USBDMA_EP6_DoneInterrupt(void) FAST_FUNC_SECTION
{	
	OS_CPU_SR cpu_sr;
	OS_ENTER_CRITICAL();	
...
void USBReadTransferCallback(void *pArg,
							 ...
							 unsigned int remaining) FAST_FUNC_SECTION
{
	INT8U err;
...
void HandleEP0Command(const USBGenericRequest *request, INT8U *  EP0INdata, INT8U * EP0OUTdata) FAST_FUNC_SECTION
{
...
//------------------------------------------------------------------------------
void USBDCallbacks_RequestReceived(const USBGenericRequest *request) FAST_FUNC_SECTION
{
	INT8U err;
...
INT8U IsSafeCommand(const USBGenericRequest *request) FAST_FUNC_SECTION
{
	INT8U ret = False;
	...
	case 0x7F:
		...
		else
		{
			if((index == 2 && value == 0) || value == 3||value == 15||value == 16 ||value == 17 ||value == 21||value == 24||value == 30||
			   value == 31||value == 32||value == 33||value == 34||value == 35||value == 36||value == 38 || value == 40 || value == 42|| value == 44|| value == 45|| value == 49|| value == 41||value == 51 || value == 52 ||
				   value == 53||value == 54||value == 55)
			{
				ret = False;
				break;
			}			
...
{	
	...
	case 0x7F:
		if (direction) //All EP0 command must have ep0 buffer, 
		{
			...
			{
				if(index == 0)
					EP0INdata[0] = MAX_HEAD_DRIVE_NUM;
				else
...
#if defined (DGEN_AUTOFUCTION)||defined(QICAIHONG)
			else if(value == 26)
			{
		...
		*len = 6;
		break;
			...
			else if (value == 38)
			{
				EP0INdata[0] = 0x7f;
				EP0INdata[1] = 38;
				*((__packed float*)&EP0INdata[2]) = LCDMenuConfig.MeasureSensorOffset / 2.54f;
				*len = 6;
				break;
			}

#if defined(COORD_NEW_UV)
			else if(value == 39)		//UV灯使用总时间
			{
				EP0INdata[0] = 0x7f;
				EP0INdata[1] = 39;
				*((__packed INT32U*)&EP0INdata[2]) = LCDMenuConfig_EX.frontUVLightOn_totalTime;
				*((__packed INT32U*)&EP0INdata[6]) = LCDMenuConfig_EX.backUVLightOn_totalTime;
				*len = 10;
				break;
			}
#endif
#if (defined(MANUFACTURER_DYSS)||defined(MANUFACTURER_DYSS_16H_FLAT))
			else if(value == 40)			//UV开启间隔PASS数
			{
				EP0INdata[0] = 0x7f;
				EP0INdata[1] = 40;
				*((INT8U*)&EP0INdata[2]) = LCDMenuConfig_EX.UVLightOnInterval_PassNum;
				*len = 3;
				
				break;
			}
			else if(value == 41)			//泵墨功能使能开关
			{
				EP0INdata[0] = 0x7f;
				EP0INdata[1] = 41;
				*((INT8U*)&EP0INdata[2]) = LCDMenuConfig_EX.isPumpFunctionOn;
				*len = 3;
				break;
			}
#endif
...
#ifdef FUNCTION_CLOSE_LCD
			else if(value == 42)		//读取校准通道名字
			{
				if(index == 0)
					*(INT32U*)EP0INdata = MAX_NOZZLE_LINE;
				else
					memcpy((void*)EP0INdata, (void*)MenuStr_LeftRightAlign_Name[index -1], MAX_HOR_NAME);
				break;
			}
#endif
#ifdef FULGENCY_FUN
			else if(value == 43)
			{
				EP0INdata[0] = 0x7f;
				EP0INdata[1] = 43;
				*((__packed float*)&EP0INdata[2]) = LCDMenuConfig_EX.MediaH;
				*len = 6;
				break;
			}
#endif
			...
			else if(value == 44)
			{
				EP0INdata[0] = 0x7f;
				EP0INdata[1] = 44;
				if(LCDMenuConfig.cfg_bit&RICOH_IDLE_FLASH)
					EP0INdata[2] = 1;
				else
					EP0INdata[2] = 0;
				*len = 3;
			...
			else if(value == 45)
			{
				EP0INdata[0] = 0x7f;
				EP0INdata[1] = 45;
				*((__packed INT32U*)&EP0INdata[2]) = epsonCaliSetting.eprCaliSetting.stepPass[LCDMenuConfig.SheetType].BaseStep;
				*len = 6;
				break;
			}
#endif
...
#ERROR
			...
			{
#if defined(DYSS_UV_CLEAN)||defined(EPSON_CLEAN_UPDOWN)||defined(RICOH_CLEAN_PRESS)		
			...
			{
				for(i = 0;i < MAX_HEAD_DRIVE_NUM;i++)
					if( (-50 <= (INT8S)EP0OUTdata[i])&&(50 >= (INT8S)EP0OUTdata[i]))
						g_headParameterPercent[i] = (INT8S)EP0OUTdata[i];
#ifdef CONVERSION_BOARD

#ifdef RICOH_G4_16H

				tmp[0] = MAX_HEAD_DRIVE_NUM+2 + 1;
				tmp[1] = UART_HEAD_EPSON_PARAM_PERCENT;
				memcpy((void *) &tmp[2], (void *)&g_headParameterPercent[0], MAX_HEAD_DRIVE_NUM/2);

				tmp[tmp[0] - 1] = 1;
				while (!UART_SendCMD(UART_HEAD_CHANNEL, tmp))
					OSTimeDly(1);

				memcpy((void *) &tmp[2], (void *)&g_headParameterPercent[8], MAX_HEAD_DRIVE_NUM/2);

				tmp[tmp[0] - 1] = 2;
				while (!UART_SendCMD(UART_HEAD_CHANNEL, tmp))
					OSTimeDly(1);
#elif defined(RICOH_G4_24H)

				tmp[0] = DRIVE_NUM_PRE_HEAD+2 + 1;
				tmp[1] = UART_HEAD_EPSON_PARAM_PERCENT;
				memcpy((void *) &tmp[2], (void *)&g_headParameterPercent[0], DRIVE_NUM_PRE_HEAD);

				tmp[tmp[0] - 1] = 1;
				while (!UART_SendCMD(UART_HEAD_CHANNEL, tmp))
					OSTimeDly(1);

				memcpy((void *) &tmp[2], (void *)&g_headParameterPercent[8], DRIVE_NUM_PRE_HEAD);

				tmp[tmp[0] - 1] = 2;
				while (!UART_SendCMD(UART_HEAD_CHANNEL, tmp))
					OSTimeDly(1);

				memcpy((void *) &tmp[2], (void *)&g_headParameterPercent[16], DRIVE_NUM_PRE_HEAD);

				tmp[tmp[0] - 1] = 3;
				while (!UART_SendCMD(UART_HEAD_CHANNEL, tmp))
					OSTimeDly(1);
#elif defined(RICOH_G5_8H)	
				tmp[0] = MAX_HEAD_DRIVE_NUM+2 + 1;
				tmp[1] = UART_HEAD_EPSON_PARAM_PERCENT;
				memcpy((void *) &tmp[2], (void *)&g_headParameterPercent[0], MAX_HEAD_DRIVE_NUM/2);
				...
				memcpy((void *) &tmp[2], (void *)&g_headParameterPercent[16], MAX_HEAD_DRIVE_NUM/2);
			...
			else if(value == 35)
			{
				...
				if(*((__packed float*)&EP0OUTdata[0]) >= -50.0f/2.54 && *((__packed float*)&EP0OUTdata[0]) <= 50.0/2.54)
				{
					LCDMenuConfig_EX.X_Plat_Correction = *((__packed float*)&EP0OUTdata[0]);
					printer.platSpace = PRINTER_PLAT_SPACE;
					AdjustPlatSpace();
				}
				if(*((__packed float*)&EP0OUTdata[4]) >= -50.0f/2.54 && *((__packed float*)&EP0OUTdata[4]) <= 50.0/2.54)
					LCDMenuConfig_EX.Y_Plat_Correction = *((__packed float*)&EP0OUTdata[4]);
		...
		if(*((__packed INT32U*)&EP0OUTdata[13]) != 0)
			...
			{
				if(index == 0)
					Y_GOHOME_Dirty |= STATUS_WATING_MASK;
				else
					Y_GOHOME_Dirty &= ~STATUS_WATING_MASK;
				break;
			}
			...
			{
				LCDMenuConfig.MeasureSensorOffset = *((__packed float*)&EP0OUTdata[0]) * 2.54f;
				SaveLCDMenuConfig();
			}
			else if(value == 50)
			{
#ifdef FULGENCY_FUN	
				CurrentPrintMode = EP0OUTdata[0];	
#endif
                break;
			}
#if defined(MANUFACTURER_DYSS_16H_FLAT)
			else if(value == 51)	//设定每N passUV亮一次
			{
				LCDMenuConfig_EX.UVLightOnInterval_PassNum = EP0OUTdata[0];
				SaveLCDMenuConfigEX_EM(&LCDMenuConfig_EX,
								   (INT8U*)&LCDMenuConfig_EX.UVLightOnInterval_PassNum - (INT8U*)&LCDMenuConfig_EX, 
								   sizeof(LCDMenuConfig_EX.UVLightOnInterval_PassNum));
				break;
			}
			else if(value == 52)	//设供墨功能开关
			{
				LCDMenuConfig_EX.isPumpFunctionOn = EP0OUTdata[0];
				if(LCDMenuConfig_EX.isPumpFunctionOn != True)
					PUMP_OPEN(0);
				SaveLCDMenuConfigEX_EM(&LCDMenuConfig_EX,
								   (INT8U*)&LCDMenuConfig_EX.isPumpFunctionOn - (INT8U*)&LCDMenuConfig_EX, 
								   sizeof(LCDMenuConfig_EX.isPumpFunctionOn));
				break;
			}
#endif
#if defined(COORD_NEW_UV)
			else if(value == 53)	//UV灯使用时间清零
			{
				if(EP0OUTdata[0] == 1)
				{
					LCDMenuConfig_EX.frontUVLightOn_totalTime = 0;
					SaveLCDMenuConfigEX_EM(&LCDMenuConfig_EX,
								   (INT8U*)&LCDMenuConfig_EX.frontUVLightOn_totalTime - (INT8U*)&LCDMenuConfig_EX, 
								   sizeof(LCDMenuConfig_EX.frontUVLightOn_totalTime));
				}
				if(EP0OUTdata[1] == 1)
				{
					LCDMenuConfig_EX.backUVLightOn_totalTime = 0;
					SaveLCDMenuConfigEX_EM(&LCDMenuConfig_EX,
								   (INT8U*)&LCDMenuConfig_EX.backUVLightOn_totalTime - (INT8U*)&LCDMenuConfig_EX, 
								   sizeof(LCDMenuConfig_EX.backUVLightOn_totalTime));
				}
				break;
			}
#endif
			...
			{
				LCDMenuConfig_EX.MediaH = *((__packed float*)&EP0OUTdata[0]);
				SaveLCDMenuConfig_EX(&LCDMenuConfig_EX,(INT8U*)&LCDMenuConfig_EX.MediaH - (INT8U*)&LCDMenuConfig_EX, sizeof(LCDMenuConfig_EX.MediaH));		
				break;
			}
#endif	
			...
			{
				if(EP0OUTdata[0] == 1)
					LCDMenuConfig.cfg_bit |= RICOH_IDLE_FLASH;
				else
					LCDMenuConfig.cfg_bit &= ~RICOH_IDLE_FLASH;
			...
			{
				epsonCaliSetting.eprCaliSetting.stepPass[LCDMenuConfig.SheetType].BaseStep = *((__packed INT32U *)&EP0OUTdata[0]);
				memset((void*)&(epsonCaliSetting.eprCaliSetting.stepPass[LCDMenuConfig.SheetType].PassStep), 0, MAX_PASS_NUMBER_CALIBRATION*2);
...
#endif
#if defined(QICAIHONG)
			Control_OpenCose_Led(3, True);
#endif
#if defined(TATE_PLATFORM_HOT)
                    OpenPlateHot();
#endif
...
#ifdef FULGENCY_FUN
			status_ReportStatus(STATUS_WAITING, STATUS_CLR);
#endif
#if defined(QICAIHONG)
			Control_OpenCose_Led(3, False);
#endif
...
#ifdef FULGENCY_FUN
		if(*((__packed INT32U *)&EP0INdata[4]) & STATUS_WAITING)
		{
			*((__packed INT32U *)&EP0INdata[4]) &= ~STATUS_WAITING;
...
#warning "need do mapping from PM UI to LCD menu"                    
	...
	case 0x69://OUT, set option password: language
		...
		else
		{
				...
				EP0INdata[0] = 0;
				EP0INdata[1] = 0x69;
				EP0INdata[2] = is_language_password;
#endif
		}
		break;
#endif	
...
#endif		
#elif (defined (EPSON_CLEAN_INTEGRATE)||defined (EPSON_CLEAN_INTEGRATE_1) ||defined(EPSON_CLEAN_INTEGRATE_2)||defined(EPSON_CLEAN_INTEGRATE_3))&&!(defined(MANUFACTURER_MICOLOR_EPSON) || defined(MANUFACTURER_COLORFUL) || defined(MANUFACTURER_ALLWIN_EPSON))
		if (EP0OUTdata[0] <= 64)
		{
...
#if defined(MANUFACTURER_DYSS)||defined(ORIC_FUNCTION)||defined(MANUFACTURER_DYSS_16H_FLAT)			
