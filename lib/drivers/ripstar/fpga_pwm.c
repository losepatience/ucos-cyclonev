...
static void __pwm_irq_handler(void *arg)
{
	...
	if(chId != 0)
	{
		__pwm_irq_empty_clr(PWM_CHID_ALL,DBUF_0);		//清中断
		__pwm_irq_empty_clr(PWM_CHID_ALL,DBUF_1);		//清中断
	}
	...
	if(chId != 0)
	{
		__pwm_irq_almost_clr(PWM_CHID_ALL,DBUF_0);		//清中断
		__pwm_irq_almost_clr(PWM_CHID_ALL,DBUF_1);		//清中断
	}
...
int pwm_init(void)
{
	...
	pwm_irq_almost_enable(PWM_CHID_ALL, 0);					//关中断
	pwm_irq_almost_mask(PWM_CHID_ALL, 1);					//关中断屏蔽
