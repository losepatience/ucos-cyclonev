...
INT8U PushHugePipeData(INT16U ArmCmd, INT8U *pBuff, INT16U huge_size)
{
	OS_CPU_SR  cpu_sr;
	INT8U err;
	INT8U ret = True;
	...
	while(huge_size > 0)
	{
		...
		if(IsHuge)
		{
			...
			data_len = (size + sizeof(INT16U)*2 + ALIGN_SIZE -1)/ALIGN_SIZE*ALIGN_SIZE;
		}
		else
		{
		...
		do{
			...
			else
			{
				ret = True;
				*(INT16U*)pRec = (INT16U)full_len;
				*(INT16U*)(pRec + sizeof(INT16U))= (INT16U)COMMPIPE_ARM_CHANNEL_ID;
				pRec += PIPEHEAD_SIZE;
				if(pRec == CacheBuff+CACHE_BUFF_SIZE)
					pRec = CacheBuff;
				*(INT16U*)pRec = (INT16U)ArmCmd;
				pRec += sizeof(INT16U);
				if(IsHuge)
				{
					*(INT16U*)(pRec)= (INT16U)offset;
					pRec += sizeof(INT16U);
				}
				if(pRec == CacheBuff+CACHE_BUFF_SIZE)
					pRec = CacheBuff;
