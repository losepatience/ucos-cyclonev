...
INT8U WAVE_NAME_BUF[4 * 8 * 8] = {0};		//һ��4��vsd��ÿ��vsd��8�����Σ���8�����֣�ÿ���������ռ8���ֽڣ���Ч7���ֽ� + 0λ����256���ֽ�
INT8U WAVE_MAP_TABLE[MAX_HEAD_DRIVE_NUM * 2] = {0};			//����Ϊͷ��ͨ������ * 2
#endif
		...
		if (headInitS1 && (!motionInitS1)&&UART_GetCMD(UART_MOTION_CHANNEL, UartCMD)) //Motion stage 1 initial OK
		{
			if ((UartCMD[0] == 6 || (UartCMD[0] == 7 && UartCMD[6] == 2)) & (UartCMD[1] == UART_INIT_STAGE1_CMD)) //Command motion start stage 2 initial
			{
				...
				if ((!valueFlag_FactoryData_Not_Init)&&(!valueFlag_BYHXData_Not_Init))
				{
					...
					UartCMD[2] = printer.xEncoder; //!0 ʹ�ù�դ�� 0 ʹ��˾������
	...
	while (1)
	{
		...
		if (UART_GetCMD(UART_HEAD_CHANNEL, UartCMD))
		{
			...
			case UART_HEAD_WAVE_CHANNEL_TRANSFER:
				...
				{
					memcpy((void*)&(WAVE_BUF[(UartCMD[3]-1)*WAVE_DATA_MAX_LEN]), (void*)&(UartCMD[4]), WAVE_DATA_MAX_LEN);
					memcpy((void*)&(WAVE_MAP_TABLE[(UartCMD[3]-1)*WAVE_DATA_MAX_LEN]), (void*)&(UartCMD[4]), WAVE_DATA_MAX_LEN);	//���ݲ���ӳ���
					DATA_LEN += WAVE_DATA_MAX_LEN;
				}
				else
				{
					memcpy((void*)&(WAVE_BUF[(UartCMD[3]-1)*WAVE_DATA_MAX_LEN]), (void*)&(UartCMD[4]), UartCMD[0] - 4);
					memcpy((void*)&(WAVE_MAP_TABLE[(UartCMD[3]-1)*WAVE_DATA_MAX_LEN]), (void*)&(UartCMD[4]), UartCMD[0] - 4);	//���ݲ���ӳ���
					DATA_LEN += (UartCMD[0] - 4);
...
#error
		...
		if(TRANSFER_MODE!=0)
		{
			...
			else if(TRANSFER_MODE == 18)  //43-25:����ӳ����·��޸�
		...
		{
			IdleClean_Time = 0;
			ActiveCleanConfig = cleanparam_EPSON_ALLWIN.AutoCleanConfig;	//������ϴǿ��
		...
		Waste_Ink_CHECK();
					...
					&&(curPos.x < 10 && curPos.x > -10)&&cleanPara.flash)
			{
				if(!flash_idle_on)
				{
					...
					if(flash_wait_50ms >= 1)//��ֵ��50����Ƶ����ͷ��fpga������ֵԽС����Խ��
					{
...
#if !defined(HEAD_EPSON_GEN5)&&	 !defined(HEAD_RICOH_G4)//RICOH��ʱ��֧��
#error
