...
static INT8U gpio_get_index(INT8U id)		//gpio������������������
{
	INT8U grp = 0;
	while(id >ngpio[grp])
	{
...
static INT16U gpio_get_grp_index(INT8U id)	//����ֵ ��8λΪ�����ţ���8λΪ������������
{
	INT8U grp = 0;
	while(id >ngpio[grp])
	{
...
#define OLD_Y_SPEED_SELECT_WAY //Ϊ�˼����ϵ��㷨��������W1615EXʱ�õ����ϳ�����
#endif

/*FIXME@motionParam_default��ʼ���޸�
 * �ṹ����Ա��SensorIOId��ʼ��ֵ��Ҫ�޸�
 * �ṹ����Ա��driveType�������ͳ�ʼ��ֵҪ�޸�,motorType/controlTypeͬ����Ҫ�޸�
 */
...
5.0827, //for micolor new machine with Ƥ����.
#endif
...
/*
 * isclear:����FPGA��PWM buffer����δִ�е�pwm,������
 */
...
void VM_PD_Print(struct AxisParam_PulseDir_OpenLoop * pPD_PWM)
{
	...
	else
	{
		...
		{
			//��ֹͣʱ��FPGA�Ļ��������ܻ���δִ���������壬�������ĵ�STOP����TCHandler_dimmy������
...
INT8U InitPWMChannel(void)
{
	...
	pwm_mode_pwm(pwm_dirCh, 0);			//����ΪIOģʽ�������ߵ͵�ƽ
	pwm_override(pwm_dirCh, 0);			//����IOĬ������Ϊ�͵�ƽ
	pwm_mode_pwm(pwm_pulseCh, 1);		//����ΪPWMģʽ������PWM����
	pwm_override(pwm_pulseCh, 0);		//��������ΪFPGA����
...
INT8U Motion_Init(void)
{
	...
	for(i = 0; i < motionParam.AxisCount; i++)
	{
		...
		if(pAxisParam->PWM_ChannelID < 2)	//X or Y
		{
			...
			//FIXME:ConfigSensorPin������������ǰ��ͬ������ʱ��Ҫ�޸�ConfigSensorPin����
...
INT8U PWM_Stop(struct AxisParam_PulseDir_OpenLoop * pPD_PWM)
{
	...
	//��ֹͣʱ��FPGA�Ļ��������ܻ���δִ���������壬���˹ر�PWM���ܷ���TCHandler_dimmy������
	if(pPD_PWM->ChannelNO == 0)
		pPD_XHandle = TCHandler_dimmy;
	else
		pPD_YHandle = TCHandler_dimmy;
...
#define CARRIAGE_WIDTH 2 //20cm
	...
	//FIXME:HEAD_EPSON_GEN5 VS HEAD_RICOH_G4 ��ͬ
...
INT8U FirstGoXHome()
{
	...
	/*todo OriginSensorPosi_X����ֵ����λ�ж��еó�����Ҫʵ����λ�����ô�ֵ
	 * ������ֵΪ0����ʾ����ԭ��λ�ò���
	 */
	...
	!defined(COLORFUL_EPSON))
		...
		while(UART_GetCMD(UART_DSP_CHANNEL, UartCMD))
		{
			...
			case UART_DSP_RPT_DESKEY:
				...
				break;
#endif						
			case UART_DSP_RPT_STOP: //Motion stop. Parameters: 4 bytes ��ǰλ�� X: INT32S ����: 4 �ֽ�, Little Endian
				OSFlagPost(mix_FLAG_GRP, MOTION_MOVE_COMPLETED_Y, OS_FLAG_SET, &err); 
...
/*
 * Q:
 * 1��PWM_Stop��PWM_Start_Movement�Ⱥ���PWM����ת����
 * 2��PWM�жϴ���������ռ�ձȸı�
 * 3����ʼʱPWM������ռ�ձ���ͬ
 * 4:PutMoveRecord����
 */
